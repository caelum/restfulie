...
h2. Following 301 response code

According to "https specification":http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html , 301 links can be automatically followed when using the *GET* and *HEAD* verbs:
...
h3. Simple usage: transitions

The most easy way to use restfulie is to pass a block to the *acts_as_restfulie* method.
...
2. Add the *acts_as_restfulie* invocation with a block returning an array of possible transitions:
...
<pre>
 def show
   ...
   respond_to do |format|
     ...
     format.xml  { render_resource @order }
   end
 end
</pre>
...
Active Commiters
* "Caue Guerra":mailto:caue.guerra@gmail.com - "twitter":http://www.twitter.com/caueguerra - "http://caueguerra.com/":http://caueguerra.com/
...
The RDoc for the latest Restfulie code is hosted on "rdoc.info":http://rdoc.info/projects/caelum/restfulie.
This documentation always reflects the HEAD of caelum's master branch and may therefore be slightly different from the latest gem release.

If you want the RDoc for a particular version of Restfulie you must build it yourself:

gem install rdoc
gem install restfulie
gem server &
open http://localhost:8808/doc_root/restfulie-0.3/rdoc
(You may have to change the version number in that URL)

h3. More tutorials

1. "blog post on the entire ecosystem in english":http://guilhermesilveira.wordpress.com/2009/11/03/quit-pretending-use-the-web-for-real-restfulie/
2. "Restfulie @ infoq":http://www.infoq.com/news/2009/11/restfulie-hypermedia-services
3. "Ruby5 podcast on restfulie":http://ruby5.envylabs.com/episodes/26-episode-25-november-6-2009/stories/201-restfulie
4. "Techstartup on restfulie":http://www.techstartups.com/2009/11/17/real-time-ruby-is-a-gem-called-restfulie/

h4. Portuguese

1. "portuguese tutorial on the server-side support":http://wakethedead.com.br/blog/70-restfulie
2. "more on restfulie - portuguese":http://andersonleiteblog.wordpress.com/2009/11/23/mais-sobre-restfulie/ and 

h4. Chinese

1. "Restfulie @ infoq":http://www.infoq.com/cn/news/2009/11/restfulie-hypermedia-services

h4. Japanese

1. "Restfulie @ infoq":http://www.infoq.com/cn/news/2009/11/restfulie-hypermedia-services?utm_source=twitterfeed&utm_medium=twitter
...
* relations instead of transitions
* lucas restfulie-vraptor support last-modified and etag
* lucas vraptor big upload check, missing contentlength check
* rails 411 check
* support lastModified and eTag on the server side automatically through render_resource
* remove client dependency from ActiveRecord
	* hashie must support boolean? fields
* controller filtering based on status
* rails 3 easier support (no controller argument!)
* support aasm and change parameter order for transition: (i.e. last one is HASH, first is name, second (if existing) is target state)
* release 0.4
